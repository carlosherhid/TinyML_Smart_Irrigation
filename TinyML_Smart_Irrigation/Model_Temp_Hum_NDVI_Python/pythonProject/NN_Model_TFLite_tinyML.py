...
plt.savefig('./modelo_neuralnetworks_worst_best_scenario.svg', format='svg')
plt.show()
...
# Guardar el gráfico como archivo .svg
plt.savefig('./modelo_redes_best.svg', format='svg')
plt.show()
...
nombre_archivo = f'modelo_{best_index}.h5'
...
with open(f'modelo_{best_index}_quantized.tflite', 'wb') as f:
    f.write(tflite_quantized_model)
# Cargamos el modelo sin cuantizar
modelo_sin_cuantizar = load_model(f'modelo_{best_index}.h5')
...
with open(f'modelo_{best_index}_quantized.tflite', 'rb') as f:
...
tamano_sin_cuantizar = os.path.getsize(f'modelo_{best_index}.h5')
tamano_cuantizado = os.path.getsize(f'modelo_{best_index}_quantized.tflite')
